...
export const FileUpload: React.FC<FileUploadProps> = ({ 
  ...
  const getIconColor = () => {
    ...
    if (isDragging) return 'text-tf-yellow';
    return 'text-gray-400';
  };

  const getIconBgColor = () => {
    if (file) return iconType === 'xml' ? 'bg-blue-100' : 'bg-red-100';
    if (isDragging) return 'bg-tf-yellow/20';
    return 'bg-gray-100';
  };
      ...
      <div className="relative">
        ...
        <label
          ...
          className={`
            ...
            ${file 
              ? 'border-tf-yellow bg-tf-yellow/10' 
              : isDragging
                ? 'border-tf-yellow bg-tf-yellow/5 scale-[1.02]'
                : 'border-gray-200 hover:border-tf-yellow/50 bg-gray-50 hover:bg-white'
            }
          `}
        >
          {/* Success indicator */}
          ...
          {file ? (
            <div className="text-center">
              <p className="text-tf-black font-semibold truncate max-w-[200px]">
                {file.name}
              </p>
            ...
            <div className="text-center">
              <p className={`font-medium ${isDragging ? 'text-tf-yellow-dark' : 'text-gray-600'}`}>
