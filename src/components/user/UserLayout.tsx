...
const UserLayout: React.FC<UserLayoutProps> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-tf-bg-main transition-colors duration-300">
      {/* Background Pattern */}
      ...
      <button
        ...
        className="lg:hidden fixed top-4 left-4 z-50 p-2.5 bg-white rounded-xl border border-tf-gray-light text-tf-black shadow-sm"
      >
      ...
      >
        {/* Logo */}
        <div className="p-5 border-b border-tf-yellow-dark">
                ...
                className={`
                  ...
                  ${
                    isActive
                      ...
                      : 'text-tf-black/80 hover:bg-tf-yellow-dark'
                  }
                `}
              >
                <Icon className={`w-5 h-5 ${isActive ? 'text-tf-yellow-accent' : 'text-tf-black/70'}`} />
                <span className="font-medium">{item.name}</span>
                {isActive && <ChevronRight className="w-4 h-4 ml-auto text-tf-yellow-accent" />}
              </Link>
            );
          })}
        </nav>
        ...
        {/* User Section */}
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-tf-yellow-dark space-y-3">
          {/* User Info */}
          <div className="flex items-center gap-3 px-2 py-2">
            <div className="w-10 h-10 bg-tf-yellow-dark rounded-xl flex items-center justify-center">
              <span className="text-tf-black font-bold">
                {user?.fiscal_name?.charAt(0) || user?.email?.charAt(0) || 'U'}
              </span>
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-tf-black font-medium truncate text-sm">
                {user?.fiscal_name || user?.email || 'Usuario'}
              </p>
              <p className="text-tf-black/60 text-xs truncate">
