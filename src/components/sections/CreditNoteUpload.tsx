    ...
    <div className="mt-6 p-5 bg-amber-50/50 rounded-2xl border border-amber-200/50">
      ...
      {validation && !isValidating && (
        <div className={`mt-4 p-4 rounded-xl border ${
          validation.isValid 
            ? 'bg-tf-green/5 border-tf-green/20' 
            : 'bg-red-50 border-red-200'
        }`}>
          <div className="flex items-start gap-3">
            {validation.isValid ? (
              <>
                <CheckCircle2 size={20} className="text-tf-green flex-shrink-0" />
                <div>
                  <p className="font-semibold text-tf-green">Nota de crédito válida</p>
                  {creditNoteData && (
