    ...
    <div className="space-y-5">
      ...
      <div className="card p-6 space-y-5">
        ...
        <div className="flex items-center justify-center min-h-[48px]">
          {isExtracting ? (
            <div className="flex items-center gap-3 text-tf-yellow-dark bg-tf-yellow/10 px-5 py-3 rounded-xl border border-tf-yellow/30 animate-pulse text-sm">
              <Loader2 size={18} className="animate-spin" />
              <span className="font-semibold">Leyendo factura y completando formulario...</span>
            </div>
          ) : extractSuccess ? (
            <div className="flex items-center gap-3 text-tf-yellow-accent bg-tf-yellow/10 px-5 py-3 rounded-xl border border-tf-yellow/30 text-sm">
              <CheckCircle size={18} />
              <span className="font-semibold">¡Datos extraídos! Verifica la información abajo.</span>
            </div>
          ) : extractError ? (
            <div className="flex items-start gap-3 text-red-600 bg-red-50 px-5 py-3 rounded-xl border border-red-200 text-sm max-w-2xl">
