...
export const Invoicing: React.FC = () => {
  ...
  const [csdLoading, setCsdLoading] = useState(true);
  ...
  // Fetch initial CSD status
  useEffect(() => {
    const fetchCsdStatus = async () => {
      setCsdLoading(true);
      try {
        const status = await getCSDStatus();
        setCsdStatus(status);
      } catch (err) {
        // If CSD fetch fails, show CSD upload section
        setCsdStatus(null);
      } finally {
        setCsdLoading(false);
      }
  ...
  const getStatusBadge = (status: string) => {
    const styles: Record<string, string> = {
      stamped: 'bg-tf-green/10 text-tf-green',
    ...
    return (
      <UserLayout>
        <div className="space-y-6">
          <div>
            <h1 className="text-2xl font-bold text-tf-black">Facturación Electrónica</h1>
  ...
  // Render when invoicing is disabled — show CSD upload prominently
        ...
        <div className="space-y-6">
          ...
          {!csdStatus?.invoicingEnabled && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-6 flex items-start gap-4">
              ...
              <div>
                <p className="font-semibold text-amber-800">Facturación no habilitada</p>
                <p className="text-sm text-amber-700 mt-1">
                  Para emitir facturas electrónicas necesitas cargar tu Certificado de Sello Digital (CSD) del SAT.
                  Sube tus archivos .cer y .key a continuación.
                </p>
              </div>
            </div>
          )}
    ...
    <UserLayout>
      ...
      {/* Filters */}
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex-1 relative">
            ...
            <input
              ...
              onChange={(e) => setSearchQuery(e.target.value)}
                ...
                className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-tf-yellow focus:border-tf-yellow"
            />
          </div>
          ...
          <select
                ...
                className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-tf-yellow focus:border-tf-yellow"
          >
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {invoices.map((invoice) => (
                      <tr key={invoice.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3">
                          <div>
                            <p className="text-sm font-medium text-tf-black truncate max-w-[200px]">
                        ...
                        <td className="px-4 py-3 text-right">
                          <span className="text-sm font-semibold text-tf-black">
              ...
              <div className="md:hidden divide-y divide-gray-200">
                {invoices.map((invoice) => (
                  <div key={invoice.id} className="p-4 space-y-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-tf-black">{invoice.receiverName || invoice.receiverRfc}</p>
      ...
      {/* Pagination */}
          ...
          <button
                      ...
                      className="p-2 text-gray-500 hover:text-tf-black disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      <ChevronLeft size={20} />
          </button>
