 ...
 * - Standard projects: Invoice between Tuesday and Thursday of CURRENT week
 * - Daily projects (Amazon): Invoice between Thursday of LAST week and Thursday of CURRENT week
...
const INVOICE_START_DAY = 2; // Tuesday - when billing period starts (standard)
const INVOICE_END_DAY = 4; // Thursday - when billing period ends
const DAILY_PERIOD_DAYS = 7; // 7 days back for daily billing projects

/**
 * Billing period type for projects
...
/**
 * Get the valid invoice DATE range based on billing period type
 * - standard: Tuesday-Thursday of CURRENT week (3 days)
 * - daily: Thursday of LAST week to Thursday of CURRENT week (7 days)
 * 
 * @param billingPeriodType - Type of billing period ('standard' | 'daily')
 */
export function getValidInvoiceDateRange(billingPeriodType: BillingPeriodType = 'standard'): { start: Date; end: Date } {
  const mexicoNow = getMexicoNow();
  const dayOfWeek = mexicoNow.getDay(); // 0 = Sunday

  if (billingPeriodType === 'daily') {
    // Daily billing: Thursday of LAST week to Thursday of CURRENT week
    // Calculate Thursday of current week
    let daysToThursday = DEADLINE_DAY - dayOfWeek;
    if (dayOfWeek === 0) {
    ...
    const thursdayCurrent = new Date(mexicoNow);
    thursdayCurrent.setDate(mexicoNow.getDate() + daysToThursday);
    ...
    thursdayLast.setDate(thursdayCurrent.getDate() - DAILY_PERIOD_DAYS);
    thursdayLast.setHours(0, 0, 0, 0);

    return { start: thursdayLast, end: thursdayCurrent };
  }

  // Standard billing: Tuesday-Thursday of CURRENT week
 ...
 * @param invoiceDate - The invoice emission date
 * @param billingPeriodType - Type of billing period ('standard' | 'daily')
 */
export function isInvoiceDateValid(invoiceDate: Date, billingPeriodType: BillingPeriodType = 'standard'): boolean {
  const { start, end } = getValidInvoiceDateRange(billingPeriodType);
 ...
 * 1. Invoice emission date must be within valid range (depends on billing period type)
 *    - standard: Tuesday-Thursday of current week
 *    - daily: Thursday of last week to Thursday of current week
 * 2. Week mentioned in description must match expected week (previous week)
 * 3. Must be uploaded before Thursday 10am deadline
 * 
 * @param invoiceDate - The invoice emission date
 * @param extractedWeekFromDescription - Week number extracted from invoice description
 * @param billingPeriodType - Type of billing period ('standard' | 'daily')
 */
export function validateInvoiceWeek(
  ...
  // Check if invoice date is within valid range (depends on billing period type)
  const validDateRange = getValidInvoiceDateRange(billingPeriodType);
 ...
 * @param billingPeriodType - Type of billing period ('standard' | 'daily')
 */
export function calculatePaymentWeek(
  invoiceDate: Date | string,
  billingPeriodType: BillingPeriodType = 'standard'
): PaymentWeekResult {
  const invoiceDateObj = typeof invoiceDate === 'string' ? new Date(invoiceDate) : invoiceDate;
  ...
  // Check valid date range (depends on billing period type)
  const validDateRange = getValidInvoiceDateRange(billingPeriodType);
  const invoiceDateValid = invoiceDateObj >= validDateRange.start && invoiceDateObj <= validDateRange.end;
 ...
 * @param billingPeriodType - Type of billing period ('standard' | 'daily')
 */
export function getValidPeriodDescription(billingPeriodType: BillingPeriodType = 'standard'): string {
  const { start, end } = getValidInvoiceDateRange(billingPeriodType);
 ...
 * @param reason - The late reason code
 * @param billingPeriodType - Type of billing period for accurate description
 */
export function getLateReasonDescription(reason: LateReason, billingPeriodType: BillingPeriodType = 'standard'): string {
  switch (reason) {
    case 'after_deadline':
      return 'Subida después del plazo límite (Jueves 10am CDMX)';
    case 'wrong_invoice_date':
      if (billingPeriodType === 'daily') {
        return 'Fecha de factura fuera del período válido (Jueves pasado a Jueves actual)';
      }
